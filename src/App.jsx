...
const LLEGIT_OPTIONS = [
  ...
  { val: "llegint", icon: "½", label: "Llegint", color: "#c8692a" },
...
const BarChart = ({ data, title, onClickBar }) => {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 11, color: "#c8692a99", letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>{title}</div>
      {data.map(d => (
        <div key={d.label} style={{ marginBottom: 6, cursor: onClickBar ? "pointer" : "default" }}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#888", marginBottom: 2 }}>
            <span style={{ color: onClickBar ? "#c8692a" : "#888" }}>{d.label}</span>
            <span style={{ color: "#c8692a" }}>{d.val}</span>
          </div>
          <div style={{ height: 6, background: "#2e1e0e", borderRadius: 3 }}>
            <div style={{ height: 6, width: `${(d.val/max)*100}%`, background: onClickBar ? "#c8692a66" : "#c8692a44", borderRadius: 3, transition: "width 0.4s" }} />
          </div>
        </div>
      ))}
    </div>
...
const EditField = ({ label, value, onSave, multiline = false, type = "text" }) => {
  ...
  if (!editing) return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 6, marginBottom: 5 }}>
      <div style={{ flex: 1 }}>
        <span style={{ color: "#c8692a66", textTransform: "uppercase", fontSize: 10, letterSpacing: 1 }}>{label} </span>
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ color: "#c8692a66", textTransform: "uppercase", fontSize: 10, letterSpacing: 1, marginBottom: 4 }}>{label}</div>
      {multiline
        ? <textarea value={val} onChange={e => setVal(e.target.value)} rows={4}
            style={{ width: "100%", background: "#1e1208", border: "1px solid #c8692a44", color: "#e8e0d0", padding: "8px", fontSize: 12, fontFamily: "Georgia, serif", borderRadius: 2, resize: "vertical", boxSizing: "border-box" }} />
        : <input type={type} value={val} onChange={e => setVal(e.target.value)}
            style={{ width: "100%", background: "#1e1208", border: "1px solid #c8692a44", color: "#e8e0d0", padding: "7px 10px", fontSize: 12, fontFamily: "Georgia, serif", borderRadius: 2, boxSizing: "border-box" }} />
      }
      <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
        <button onClick={save} style={{ padding: "4px 12px", background: "#c8692a22", border: "1px solid #c8692a44", color: "#c8692a", cursor: "pointer", fontSize: 11, borderRadius: 2, fontFamily: "Georgia, serif" }}>Desar</button>
...
// ── STYLES ──
const S = {
  app: { minHeight: "100vh", background: "#130c06", color: "#e8e0d0", fontFamily: "Georgia, serif", maxWidth: "100%", fontSize: 16 },
  header: { padding: "14px 16px 0", background: "#130c06", borderBottom: "1px solid #1a2540", position: "sticky", top: 0, zIndex: 100 },
  headerTop: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 },
  title: { fontSize: 17, fontWeight: "bold", letterSpacing: 3, color: "#c8692a", textTransform: "uppercase", margin: 0, display: "flex", alignItems: "center" },
  headerRight: { display: "flex", gap: 8, alignItems: "center" },
  iconBtn: { background: "none", border: "1px solid #2a3a5a", color: "#b89070", cursor: "pointer", fontSize: 13, padding: "3px 9px", borderRadius: 12, fontFamily: "Georgia, serif" },
  nav: { display: "flex" },
  navBtn: (a) => ({ flex: 1, padding: "9px 4px", background: "none", color: a ? "#c8692a" : "#a07850", border: "none", borderBottom: `2px solid ${a ? "#c8692a" : "transparent"}`, cursor: "pointer", fontSize: 13, fontFamily: "Georgia, serif", letterSpacing: 1, textTransform: "uppercase" }),
  searchWrap: { padding: "10px 16px", background: "#130c06", borderBottom: "1px solid #1a2540", position: "sticky", top: 83, zIndex: 99 },
  input: { width: "100%", background: "#1e1208", border: "1px solid #2a3a5a", color: "#e8e0d0", padding: "9px 12px", fontSize: 15, fontFamily: "Georgia, serif", outline: "none", boxSizing: "border-box", borderRadius: 2 },
  pills: { display: "flex", gap: 6, overflowX: "auto", padding: "8px 16px", scrollbarWidth: "none", borderBottom: "1px solid #1a2540" },
  pill: (a) => ({ padding: "4px 10px", background: a ? "#c8692a18" : "transparent", color: a ? "#c8692a" : "#a07850", border: `1px solid ${a ? "#c8692a55" : "#3a2010"}`, cursor: "pointer", fontSize: 11, whiteSpace: "nowrap", fontFamily: "Georgia, serif", borderRadius: 2 }),
  statsBar: { padding: "6px 16px", fontSize: 13, color: "#a07850", borderBottom: "1px solid #111", background: "#0e0a04" },
  viewToggle: { display: "flex", gap: 4 },
  viewBtn: (a) => ({ padding: "3px 8px", background: a ? "#c8692a22" : "transparent", color: a ? "#c8692a" : "#a07850", border: `1px solid ${a ? "#c8692a44" : "#3a2010"}`, cursor: "pointer", fontSize: 11, borderRadius: 2 }),
  grid: (cols) => ({ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 2, padding: 2 }),
  card: { position: "relative", aspectRatio: "2/3", overflow: "hidden", cursor: "pointer", background: "#1e1208" },
  ...
  cardShelf: { fontSize: 8, color: "#c8692a", marginTop: 2 },
  cardLlegit: { position: "absolute", top: 5, left: 5, fontSize: 12, textShadow: "0 0 4px black" },
  multiDot: { position: "absolute", top: 5, right: 5, background: "#c8692a", color: "#130c06", fontSize: 8, fontWeight: "bold", padding: "1px 5px", borderRadius: 8 },
  listItem: { display: "flex", gap: 12, padding: "12px 16px", borderBottom: "1px solid #131f35", cursor: "pointer", alignItems: "center" },
  listImg: { width: 38, height: 57, objectFit: "cover", background: "#1e1208", flexShrink: 0 },
  listInfo: { flex: 1, minWidth: 0 },
  listTitle: { fontSize: 16, color: "#eee8d8", marginBottom: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" },
  listMeta: { fontSize: 13, color: "#a07850" },
  listShelf: { fontSize: 12, color: "#c8692a", fontFamily: "monospace", flexShrink: 0 },
  listLlegit: { fontSize: 14, flexShrink: 0 },
  loader: { textAlign: "center", padding: 60, color: "#a07850", fontSize: 12, letterSpacing: 3 },
  empty: { textAlign: "center", padding: 40, color: "#a07850", fontSize: 14 },
  errMsg: { textAlign: "center", padding: 30, color: "#c44", fontSize: 13, lineHeight: 1.5 },
  detall: { padding: 20 },
  backBtn: { background: "none", border: "none", color: "#c8692a", cursor: "pointer", fontSize: 14, fontFamily: "Georgia, serif", padding: "0 0 14px 0" },
  edTabs: { display: "flex", gap: 4, marginBottom: 16, flexWrap: "wrap" },
  edTab: (a) => ({ padding: "5px 12px", background: a ? "#c8692a18" : "transparent", color: a ? "#c8692a" : "#a07850", border: `1px solid ${a ? "#c8692a55" : "#3a2010"}`, cursor: "pointer", fontSize: 11, fontFamily: "Georgia, serif", borderRadius: 2 }),
  detallImgWrap: { position: "relative", marginBottom: 12 },
  detallImg: { width: "100%", maxHeight: 280, objectFit: "contain", background: "#1e1208", display: "block" },
  ...
  detallOrig: { fontSize: 16, color: "#b89070", fontStyle: "italic", marginBottom: 10 },
  resum: { fontSize: 15, color: "#ccc8b8", lineHeight: 1.7, marginBottom: 12, padding: "14px 16px", background: "#1e1208", borderLeft: "3px solid #c8692a55" },
  llegitRow: { display: "flex", gap: 6, alignItems: "center", marginBottom: 16, padding: "10px 14px", background: "#1e1208", borderRadius: 2, flexWrap: "wrap" },
  llegitBtn: (active, color) => ({ padding: "5px 12px", background: active ? color + "22" : "transparent", color: active ? color : "#a07850", border: `1px solid ${active ? color + "66" : "#3a2010"}`, cursor: "pointer", fontSize: 13, borderRadius: 2, fontFamily: "Georgia, serif" }),
  starsRow: { display: "flex", gap: 4, alignItems: "center", marginLeft: "auto" },
  star: (active) => ({ fontSize: 18, cursor: "pointer", color: active ? "#c8692a" : "#3a2010" }),
  sectionTitle: { fontSize: 13, color: "#c8692acc", letterSpacing: 2, textTransform: "uppercase", margin: "20px 0 10px", borderBottom: "1px solid #2a3a5a", paddingBottom: 6 },
  badge: { display: "inline-block", padding: "3px 8px", background: "#c8692a12", border: "1px solid #c8692a44", color: "#c8692a", fontSize: 13, marginRight: 4, marginBottom: 4, letterSpacing: 0.5, borderRadius: 2 },
  notesArea: { width: "100%", background: "#1e1208", border: "1px solid #2a3a5a", color: "#e8e0d0", padding: "10px 12px", fontSize: 13, fontFamily: "Georgia, serif", borderRadius: 2, resize: "vertical", boxSizing: "border-box", minHeight: 80 },
  ...
  selBtn: (a) => ({ padding: "5px 10px", background: a ? "#c8692a" : "transparent", color: a ? "#130c06" : "#b89070", border: `1px solid ${a ? "#c8692a" : "#3a2010"}`, cursor: "pointer", fontSize: 11, fontFamily: "Georgia, serif", borderRadius: 2 }),
  ...
  faltenTitle: { fontSize: 14, color: "#eee8d8", marginBottom: 2 },
  faltenCast: { fontSize: 12, color: "#c8692a99", fontStyle: "italic", marginBottom: 2 },
  faltenYear: { fontSize: 13, color: "#a07850" },
  tipusBadge: { fontSize: 10, color: "#c8692a", border: "1px solid #c8692a55", padding: "2px 7px", whiteSpace: "nowrap", borderRadius: 2, flexShrink: 0 },
  relatItem: { padding: "14px 16px", borderBottom: "1px solid #1a2540" },
  relatTitle: { fontSize: 14, color: "#eee8d8", marginBottom: 4, fontStyle: "italic" },
  relatMeta: { fontSize: 12, color: "#a07850", marginBottom: 5 },
  relatResum: { fontSize: 12, color: "#aaa8a0", lineHeight: 1.6, marginBottom: 6 },
  relatBooks: { display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 },
  relatBook: { fontSize: 10, color: "#c07840", border: "1px solid #8899cc44", padding: "2px 7px", borderRadius: 2 },
  ...
  univers: { display: "inline-block", fontSize: 9, color: "#c8692a99", border: "1px solid #c8692a22", padding: "1px 6px", marginLeft: 6, borderRadius: 2 },
  prestSection: { padding: "14px 16px 5px", borderBottom: "1px solid #1a2540" },
  prestSectionTitle: { fontSize: 13, color: "#c8692a", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 },
  prestItem: { padding: "12px 16px", borderBottom: "1px solid #131f35", display: "flex", gap: 12, alignItems: "center", cursor: "pointer" },
  prestShelf: { fontSize: 12, color: "#c8692a", minWidth: 110, fontFamily: "monospace" },
  prestTitle: { fontSize: 15, color: "#eee8d8", flex: 1 },
  prestLlegit: { fontSize: 15 },
  statsPage: { padding: 20 },
  statsTitle: { fontSize: 15, color: "#c8692a", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20, marginTop: 0 },
  statGrid: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10, marginBottom: 24 },
  statCard: { background: "#1e1208", border: "1px solid #1a2540", borderRadius: 4, padding: "14px 16px", cursor: "pointer" },
  statNum: { fontSize: 28, color: "#c8692a", fontWeight: "bold", lineHeight: 1 },
  statLabel: { fontSize: 13, color: "#a07850", marginTop: 4 },
  ...
  modal: { background: "#1e1208", border: "1px solid #2a3a5a", borderRadius: 6, padding: 24, maxWidth: 480, width: "100%", maxHeight: "90vh", overflowY: "auto" },
  modalTitle: { fontSize: 16, color: "#c8692a", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16, marginTop: 0 },
  modalRow: { fontSize: 15, color: "#aab0c0", marginBottom: 8, display: "flex", justifyContent: "space-between" },
  modalVal: { color: "#eee8d8", fontWeight: "bold" },
  modalClose: { width: "100%", marginTop: 8, padding: "10px", background: "none", border: "1px solid #2a3a5a", color: "#b89070", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 12, borderRadius: 2 },
  modalDivider: { borderTop: "1px solid #1a2540", margin: "14px 0" },
  modalSmall: { fontSize: 10, color: "#a07850", textAlign: "center", marginTop: 8 },
  formGroup: { marginBottom: 14 },
  formLabel: { fontSize: 12, color: "#c8692aaa", textTransform: "uppercase", letterSpacing: 1, display: "block", marginBottom: 4 },
  formInput: { width: "100%", background: "#130c06", border: "1px solid #2a3a5a", color: "#e8e0d0", padding: "8px 10px", fontSize: 15, fontFamily: "Georgia, serif", borderRadius: 2, boxSizing: "border-box", outline: "none" },
  formBtn: { width: "100%", padding: "11px", background: "#c8692a22", border: "1px solid #c8692a55", color: "#c8692a", cursor: "pointer", fontFamily: "Georgia, serif", fontSize: 13, borderRadius: 2, marginTop: 8 },
  addBtn: { position: "fixed", bottom: 20, right: 20, background: "#c8692a", color: "#130c06", border: "none", borderRadius: "50%", width: 52, height: 52, fontSize: 26, cursor: "pointer", zIndex: 150, boxShadow: "0 4px 16px rgba(200,169,110,0.4)", display: "flex", alignItems: "center", justifyContent: "center" },
};
      ...
      <div style={S.modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div style={S.formGroup}>
            ...
            <div style={{ display:"flex", gap:6 }}>
              ...
              <button style={{ padding:"6px 10px", background:"#c8692a22", border:"1px solid #c8692a44", color:"#c8692a", cursor:"pointer", borderRadius:2, fontSize:11 }}
            ...
            {scanning && (
              <div style={{ marginTop:8, position:"relative", background:"#000", borderRadius:8, overflow:"hidden" }}>
                ...
                <div style={{ position:"absolute", top:0, left:0, right:0, bottom:0, pointerEvents:"none" }}>
                  <div style={{ position:"absolute", top:"50%", left:"10%", right:"10%", height:2, background:"#c8692a88", boxShadow:"0 0 8px #c8692a" }} />
                  <div style={{ position:"absolute", top:"30%", left:"10%", right:"10%", bottom:"30%", border:"2px solid #c8692a44", borderRadius:4 }} />
                </div>
                <div style={{ padding:"6px 12px", background:"rgba(0,0,0,0.7)", textAlign:"center" }}>
                  <div style={{ fontSize:10, color:"#c8692a", marginBottom:4 }}>📷 Apunta el codi de barres al requadre</div>
          ...
          <div style={S.formGroup}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {["paper","ebook"].map(f => (
                <button key={f} style={{ flex:1, padding:"6px", background: form.format===f?"#c8692a22":"transparent", border:`1px solid ${form.format===f?"#c8692a44":"#2e1a08"}`, color: form.format===f?"#c8692a":"#555", cursor:"pointer", fontSize:11, borderRadius:2, fontFamily:"Georgia, serif" }}
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            ...
            <button style={{ background: "none", border: "1px solid #c8692a33", color: "#c8692a88", cursor: "pointer", fontSize: 10, padding: "2px 8px", borderRadius: 2, fontFamily: "Georgia, serif" }}
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.header}>
        <div style={S.headerTop}>
          <svg width="200" height="50" viewBox="0 0 400 100" xmlns="http://www.w3.org/2000/svg" style={{display:"block"}}>
            ...
            <line x1="85" y1="15" x2="85" y2="85" stroke="#c8692a44" strokeWidth="1"/>
            <text x="100" y="48" fontFamily="Georgia, serif" fontSize="28" fontWeight="bold" fill="#c8692a" letterSpacing="4">BIBLIOTECA</text>
            ...
            <line x1="100" y1="55" x2="395" y2="55" stroke="#c8692a44" strokeWidth="0.5"/>
            ...
            <line x1="100" y1="88" x2="210" y2="88" stroke="#c8692a" strokeWidth="2"/>
          </svg>
          ...
          <div style={S.modalOverlay} onClick={() => setShowManteniment(false)}>
            <div style={{...S.modal, maxWidth:520}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:16}}>
                {[
                  {label:"📸 Sense preu", val:sensePreu.length, color: sensePreu.length>0?"#c8692a":"#6ec88e"},
                  {label:"📄 Sense pàgines", val:sensePagines.length, color: sensePagines.length>50?"#c44":"#c8692a"},
                  {label:"🔤 Sense traductor", val:senseTraductor.length, color:"#c07840"},
                  {label:"🔢 Sense ISBN", val:senseIsbn.length, color:"#a07850"},
                  ...
                  <div key={s.label} style={{background:"#0e0a04", border:`1px solid ${s.color}44`, borderRadius:4, padding:"10px 12px"}}>
                    <div style={{fontSize:20, color:s.color, fontWeight:"bold"}}>{s.val}</div>
                    <div style={{fontSize:11, color:"#a07850", marginTop:2}}>{s.label}</div>
                  </div>
                ))}
              </div>
            ...
            <div style={{...S.modal, maxWidth:500}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{ display:"flex", gap:6, marginBottom:16 }}>
                {[{id:"NES",label:"Nesbø"},{id:"LAR",label:"Larsson"},{id:"MAN",label:"Mankell"},{id:"CHR",label:"Christie"}].map(s => (
                  <button key={s.id}
                    style={{ flex:1, padding:"6px", background:tourSeccio===s.id?"#c8692a18":"transparent", border:`1px solid ${tourSeccio===s.id?"#c8692a55":"#2e1a08"}`, color:tourSeccio===s.id?"#c8692a":"#555", cursor:"pointer", fontSize:12, fontFamily:"Georgia, serif", borderRadius:2 }}
              ...
              {tourActual.map((item, i) => {
                ...
                return (
                  <div key={i} style={{ display:"flex", gap:10, padding:"10px 0", borderBottom:"1px solid #1a2540", opacity: llegit ? 0.5 : 1 }}>
                    <div style={{ flexShrink:0, width:22, height:22, borderRadius:"50%", background: llegit?"#6ec88e22":tinc?"#c8692a18":"#2e1e0e", border:`1px solid ${llegit?"#6ec88e":tinc?"#c8692a":"#333"}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, color:llegit?"#6ec88e":tinc?"#c8692a":"#555" }}>
                      {llegit ? "✓" : i+1}
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:12, color: llegit?"#555":tinc?"#e8e0d0":"#888", marginBottom:2 }}>{item.titol}</div>
                      <div style={{ fontSize:10, color:"#444", fontStyle:"italic" }}>{item.nota}</div>
                    </div>
                    <div style={{ flexShrink:0, fontSize:10, color: llegit?"#6ec88e":tinc?"#c8692a":"#444" }}>
        ...
        const ItemPreu = ({ed, seccio}) => {
          ...
          return (
            <div style={{ display:"flex", gap:10, alignItems:"center", padding:"8px 0", borderBottom:"1px solid #1a2540" }}>
              <img src={getCover(ed)} style={{ width:28, height:42, objectFit:"cover", flexShrink:0, background:"#1e1208" }} onError={e=>{e.target.src=COVER_FALLBACK;}} />
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:12, color:"#e8e0d0", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{ed.titulo_edicion||ed.titulo_original}</div>
                <div style={{ fontSize:10, color:"#555" }}>
                  {ed.id_estanteria}
                  {ed.preu_mercat && <span style={{ color:"#c8692a88", marginLeft:6 }}>{ed.preu_mercat}€</span>}
                  {dataStr && <span style={{ color:"#444", marginLeft:4 }}>· {dataStr}</span>}
                </div>
              </div>
              <button style={{ padding:"3px 7px", background:"#1e1208", border:"1px solid #1e3060", color:"#666", cursor:"pointer", fontSize:9, borderRadius:2, flexShrink:0 }}
              ...
              <input
                ...
                style={{ width:65, background:"#130c06", border:"1px solid #1e3060", color:"#e8e0d0", padding:"4px 6px", fontSize:11, fontFamily:"Georgia, serif", borderRadius:2, textAlign:"right" }} />
              <button
                style={{ padding:"3px 8px", background: val?"#c8692a22":"transparent", border:`1px solid ${val?"#c8692a44":"#2e1a08"}`, color:val?"#c8692a":"#333", cursor:"pointer", fontSize:11, borderRadius:2, flexShrink:0 }}
        ...
        return (
          <div style={S.modalOverlay} onClick={() => setShowRevisioPreus(false)}>
            <div style={{...S.modal, maxWidth:520}} onClick={e=>e.stopPropagation()}>
              ...
              {/* Preus antics */}
              {preuAntic.length > 0 && <>
                <div style={{ fontSize:10, color:"#c8692a66", letterSpacing:2, textTransform:"uppercase", marginBottom:8 }}>
              ...
              {/* Sense preu */}
              {sensePreu.length > 0 && <>
                <div style={{ fontSize:10, color:"#c8692a66", letterSpacing:2, textTransform:"uppercase", margin:"16px 0 8px" }}>
      ...
      {showAbout && (
        <div style={S.modalOverlay} onClick={() => setShowAbout(false)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            <button style={{...S.modalClose, color: "#c8692a66", borderColor: "#c8692a22"}} onClick={exportCSV}>📤 Exportar CSV</button>
            <button style={{...S.modalClose, color: "#c07840", borderColor: "#8899cc44", marginBottom:4}} onClick={() => { setShowAbout(false); setShowTour(true); }}>🗺 Tour de lectura</button>
            {autenticat && <button style={{...S.modalClose, color: "#c8692a", borderColor: "#c8692a44", marginBottom:4}} onClick={() => { setShowAbout(false); setShowRevisioPreus(true); }}>💰 Revisar preus</button>}
        ...
        <div style={S.modalOverlay} onClick={() => setShowAutorModal(false)}>
          <div style={{ ...S.modal, maxWidth: 520 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
              <div>
                <div style={{ fontSize:18, color:"#c8692a", fontWeight:"bold", letterSpacing:1 }}>{autorModal.autor}</div>
            ...
            {/* Bio */}
            {autorModal.bio
              ? <p style={{ fontSize:12, color:"#bbb", lineHeight:1.7, marginBottom:14, padding:"10px 12px", background:"#1e1208", borderLeft:"2px solid #c8692a44" }}>{autorModal.bio}</p>
            ...
            {/* Obra completa */}
            <div style={{ fontSize:11, color:"#c8692a99", letterSpacing:2, textTransform:"uppercase", marginBottom:12 }}>Obra completa</div>
              ...
              ? OBRA_COMPLETA[autorModal.autor_id].map(g => {
                  ...
                  return (
                    <div key={g.grup} style={{ marginBottom:14 }}>
                      <div style={{ fontSize:10, color:"#c8692a55", letterSpacing:2, textTransform:"uppercase", marginBottom:6 }}>{g.grup}</div>
                      {g.items.map(item => {
                        ...
                        return (
                          <div key={item.titol} style={{ display:"flex", alignItems:"center", gap:8, padding:"4px 0", borderBottom:"1px solid #1a254008" }}>
                            ...
                            <span style={{ fontSize:12, color: tinc ? "#e8e0d0" : "#555" }}>{item.titol}</span>
      ...
      {view === "biblioteca" && <>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 16px", borderBottom:"1px solid #1a2540", position:"sticky", top:83, zIndex:98, background:"#130c06", overflow:"visible" }}>

          {/* Selector secció — dropdown */}
          <div data-seccio-menu style={{ position:"relative", flexShrink:0, zIndex:500 }}>
            <button
              style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 12px", background: seccio!=="ALL" ? "#c8692a18" : "transparent", border:`1px solid ${seccio!=="ALL" ? "#c8692a55" : "#2e1a08"}`, color: seccio!=="ALL" ? "#c8692a" : "#666", cursor:"pointer", fontSize:11, fontFamily:"Georgia, serif", borderRadius:2, whiteSpace:"nowrap" }}
              onClick={() => setShowSeccioMenu(m => !m)}>
              {getSeccioLabel(seccio)}
              <span style={{ fontSize:9, opacity:0.6 }}>{showSeccioMenu ? "▲" : "▼"}</span>
            </button>
            {showSeccioMenu && (
              <div style={{ position:"absolute", top:"calc(100% + 4px)", left:0, background:"#1e1208", border:"1px solid #1e3060", borderRadius:4, zIndex:500, minWidth:220, boxShadow:"0 8px 24px rgba(0,0,0,0.6)", padding:"6px 0" }}
                onClick={e => e.stopPropagation()}>
                <button style={{ display:"block", width:"100%", textAlign:"left", padding:"7px 14px", background: seccio==="ALL" ? "#c8692a18" : "none", color: seccio==="ALL" ? "#c8692a" : "#aaa", border:"none", cursor:"pointer", fontSize:12, fontFamily:"Georgia, serif" }}
                ...
                {SECCIONS_GRUPS.map(g => (
                  <div key={g.grup}>
                    <div style={{ padding:"5px 14px 3px", fontSize:9, color:"#c8692a55", letterSpacing:2, textTransform:"uppercase" }}>{g.grup}</div>
                    {g.items.map(s => (
                      <button key={s.id}
                        style={{ display:"block", width:"100%", textAlign:"left", padding:"6px 14px 6px 22px", background: seccio===s.id ? "#c8692a18" : "none", color: seccio===s.id ? "#c8692a" : "#888", border:"none", cursor:"pointer", fontSize:12, fontFamily:"Georgia, serif" }}
          ...
          <input
            style={{ flex:1, background:"#1e1208", border:"1px solid #1e3060", color:"#e8e0d0", padding:"6px 10px", fontSize:12, fontFamily:"Georgia, serif", outline:"none", borderRadius:2, minWidth:0 }}
          ...
          {[
            ...
            return (
              <button key={f.val} title={f.title}
                style={{ padding:"4px 8px", background: active ? "#c8692a18" : "transparent", border:`1px solid ${active ? "#c8692a55" : "#2e1a08"}`, color: active ? "#c8692a" : "#555", cursor:"pointer", fontSize:13, borderRadius:2, flexShrink:0 }}
                onClick={() => {
        ...
        <div style={{ ...S.statsBar, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span>
            ...
            {filtreExtra && <span style={{ color: "#c8692a", marginLeft: 8 }}>· {filtreExtra.valor} <button onClick={() => setFiltreExtra(null)} style={{ background: "none", border: "none", color: "#c44", cursor: "pointer", fontSize: 10 }}>✕</button></span>}
          </span>
                ...
                return (
                  <div key={ed.obra_id} style={{...S.listItem, borderLeft: getSaga(ed.id_estanteria) ? `3px solid ${getSaga(ed.id_estanteria).color}` : "3px solid transparent"}} onClick={() => openObra(ed)}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:4, flexShrink:0 }}>
                      {isPrimeraEdicio(ed) && <span style={{ fontSize:8, background:"#c8692a", color:"#130c06", padding:"1px 4px", fontWeight:"bold", borderRadius:1 }}>1ª</span>}
                ...
                {gridItems.map(ed => {
                  ...
                  return (
                    <div key={ed.obra_id} style={S.card} onClick={() => openObra(ed)}>
                      ...
                      {ed.prestat_a && <div style={{ position:"absolute", top:5, left:5, fontSize:10, background:"rgba(0,0,0,0.7)", color:"#c8692a", padding:"1px 4px", borderRadius:2 }}>🔄</div>}
                      ...
                      {ed.estat_exemplar==="venut" && <div style={{ position:"absolute", top:5, right:5, fontSize:9, background:"rgba(200,169,110,0.9)", color:"#130c06", padding:"1px 5px", borderRadius:2, fontWeight:"bold" }}>💰</div>}
                      {ed.estat_exemplar==="regalat" && <div style={{ position:"absolute", top:5, right:5, fontSize:9, background:"rgba(136,153,204,0.9)", color:"#130c06", padding:"1px 5px", borderRadius:2, fontWeight:"bold" }}>🎁</div>}
                      {isPrimeraEdicio(ed) && (
                        <div style={{ position:"absolute", inset:0, border:"2px solid #c8692a", pointerEvents:"none", borderRadius:0, zIndex:2 }} />
                      )}
                      {isPrimeraEdicio(ed) && (
                        <div style={{ position:"absolute", bottom:38, left:4, background:"#c8692a", color:"#130c06", fontSize:7, fontWeight:"bold", padding:"2px 5px", letterSpacing:0.5, zIndex:3, borderRadius:1 }}>1ª ED</div>
                      )}
      ...
      {view === "detall" && selectedObra && activeEd && (
        <div style={S.detall}>
          ...
          <div style={{ display:"flex", gap:24, alignItems:"flex-start", flexWrap:"wrap" }}>
            ...
            <div style={{ flexShrink:0, width:"min(240px, 100%)" }}>
              <div style={S.detallImgWrap}>
                ...
                {isPrimeraEdicio(activeEd) && <div style={{ position:"absolute", top:0, right:0, background:"#c8692a", color:"#130c06", fontSize:9, fontWeight:"bold", padding:"3px 7px" }}>1ª ED</div>}
              </div>
              <div style={S.coverBtns}>
                ...
                {autenticat && <>
                  ...
                  <button style={S.coverBtn("#c07840")} onClick={() => handleAutoFindCover(activeEd)} disabled={autoFindLoading}>{autoFindLoading?"⏳":"🔎 Auto"}</button>
                  <button style={S.coverBtn("#7799aa")} onClick={() => buscarPortada(activeEd)}>🌐 Google</button>
                  <button style={S.coverBtn("#c8692a")} onClick={() => setShowCoverInput(!showCoverInput)}>🔗 URL</button>
                </>}
                {autenticat && (
              ...
              {showCoverInput && (
                <div style={{display:"flex", gap:6, marginBottom:12}}>
                  ...
                  <button style={{padding:"6px 12px", background:"#c8692a22", border:"1px solid #c8692a44", color:"#c8692a", cursor:"pointer", borderRadius:2}} onClick={() => handleSaveCustomCover(activeEd.edicion_id, coverInputVal)}>✓</button>
                </div>
              )}
            </div>
            ...
            <div style={{flex:1, minWidth:240}}>
              ...
              {activeEd.prestat_a && <div style={{background:"#c8692a11", border:"1px solid #c8692a33", padding:"6px 12px", borderRadius:2, fontSize:13, color:"#c8692a", marginBottom:10}}>🔄 Prestat a <strong>{activeEd.prestat_a}</strong>{activeEd.prestat_data && ` · des de ${new Date(activeEd.prestat_data).toLocaleDateString('ca-ES')}`}</div>}

              {activeEd.resum && <div style={S.resum}>{activeEd.resum}</div>}
              {!showResumPreview && (
                <button style={{background:"none", border:"1px solid #c8692a22", color:"#c8692a66", cursor:"pointer", fontSize:12, padding:"4px 12px", borderRadius:2, fontFamily:"Georgia, serif", marginBottom:12}}
              ...
              {showResumPreview && autoResumPreview && (
                <div style={{background:"#0e0a04", border:"1px solid #c8692a33", padding:14, borderRadius:4, marginBottom:12}}>
                  <div style={{fontSize:12, color:"#c8692a66", letterSpacing:1, textTransform:"uppercase", marginBottom:8}}>✨ Resum suggerit</div>
                  <div style={{fontSize:14, color:"#bbb", lineHeight:1.6, marginBottom:10}}>{autoResumPreview}</div>
                  <div style={{display:"flex", gap:8}}>
                    <button style={{flex:1, padding:"8px", background:"#c8692a22", border:"1px solid #c8692a44", color:"#c8692a", cursor:"pointer", fontSize:13, borderRadius:2, fontFamily:"Georgia, serif"}} onClick={() => handleAcceptResum(activeEd.edicion_id, autoResumPreview)}>✓ Acceptar</button>
                    <button style={{flex:1, padding:"8px", background:"none", border:"1px solid #2a3a5a", color:"#a07850", cursor:"pointer", fontSize:13, borderRadius:2, fontFamily:"Georgia, serif"}} onClick={() => setShowResumPreview(false)}>✕ Descartar</button>
                  </div>
                </div>
              )}
              ...
              {/* Info bibliogràfica */}
              {activeEd.autor_id && (
                <div style={{fontSize:15, color:"#d0cdc0", marginBottom:7}}>
                  <span style={{color:"#c8692acc", fontSize:13, letterSpacing:1, textTransform:"uppercase"}}>Autor </span>
                  <button style={{background:"none", border:"none", color:"#c8692a", cursor:"pointer", fontSize:14, fontFamily:"Georgia, serif", padding:0, textDecoration:"underline dotted"}}
              ...
              {activeEd.editorial && <div style={{fontSize:15, color:"#d0cdc0", marginBottom:7}}><span style={{color:"#c8692acc", fontSize:13, letterSpacing:1, textTransform:"uppercase"}}>Editorial </span>{activeEd.editorial}{activeEd.coleccion && ` · ${activeEd.coleccion}`}</div>}
              {(activeEd.ano_edicion||activeEd.ano_obra) && <div style={{fontSize:15, color:"#d0cdc0", marginBottom:7}}><span style={{color:"#c8692acc", fontSize:13, letterSpacing:1, textTransform:"uppercase"}}>Edició </span>{activeEd.ano_edicion||"—"}{activeEd.ano_obra && ` · Obra: ${activeEd.ano_obra}`}</div>}
              {(activeEd.edicio_numero||activeEd.edicio_especial||activeEd.numeracio) && (
                <div style={{display:"flex", gap:6, flexWrap:"wrap", marginBottom:8}}>
                  {activeEd.edicio_numero && <span style={{fontSize:12, background:"#c8692a18", border:"1px solid #c8692a33", color:"#c8692a", padding:"2px 8px", borderRadius:2}}>#{activeEd.edicio_numero}ª edició</span>}
                  {activeEd.edicio_especial && <span style={{fontSize:12, background:"#8899cc18", border:"1px solid #8899cc33", color:"#c07840", padding:"2px 8px", borderRadius:2}}>✦ {activeEd.edicio_especial}</span>}
                  {activeEd.numeracio && <span style={{fontSize:12, background:"#c8692a18", border:"1px solid #c8692a33", color:"#c8692a99", padding:"2px 8px", borderRadius:2}}>Nº {activeEd.numeracio}</span>}
                </div>
              )}

              {/* Valor mercat */}
              <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", margin:"16px 0 8px", padding:"14px 16px", background:"#1e1208", borderRadius:4}}>
                <div>
                  <div style={{fontSize:12, color:"#c8692acc", letterSpacing:1, textTransform:"uppercase", marginBottom:4}}>Valor mercat</div>
                  <div style={{fontSize:28, color:"#c8692a", fontWeight:"bold"}}>{activeEd.preu_mercat?`${activeEd.preu_mercat}€`:"—"}</div>
                  {activeEd.preu_mercat_anterior && <div style={{fontSize:12, color:"#a07850"}}>Anterior: {activeEd.preu_mercat_anterior}€</div>}
                </div>
                <div style={{display:"flex", flexDirection:"column", gap:8}}>
                  <button style={{padding:"10px 16px", background:"#c8692a22", border:"1px solid #c8692a44", color:"#c8692a", cursor:"pointer", borderRadius:4, fontFamily:"Georgia, serif", fontSize:14}}
                    onClick={() => requireAuth(() => setShowRevaluar(true))}>💰 Revaluar</button>
                  <button style={{padding:"10px 16px", background:"none", border:"1px solid #2a3a5a", color:"#b89070", cursor:"pointer", borderRadius:4, fontFamily:"Georgia, serif", fontSize:14}}
              ...
              {showRevaluar && (
                <div style={{display:"flex", gap:8, marginBottom:12}}>
                  <input type="text" inputMode="decimal" placeholder="Nou preu €" value={nouPreu} onChange={e => setNouPreu(e.target.value)}
                    style={{flex:1, background:"#130c06", border:"1px solid #2a3a5a", color:"#e8e0d0", padding:"8px 12px", fontSize:14, fontFamily:"Georgia, serif", borderRadius:2}} />
                  <button style={{padding:"8px 14px", background:"#c8692a22", border:"1px solid #c8692a44", color:"#c8692a", cursor:"pointer", borderRadius:2, fontFamily:"Georgia, serif"}}
                    onClick={() => handleRevaluar(activeEd.edicion_id, nouPreu)} disabled={savingPreu}>{savingPreu?"...":"✓"}</button>
                  <button style={{padding:"8px 12px", background:"none", border:"1px solid #2a3a5a", color:"#a07850", cursor:"pointer", borderRadius:2}}
              ...
              {showPreuMenu && (
                <div style={{display:"flex", gap:6, flexWrap:"wrap", marginBottom:12}}>
                  {[{label:"📚 Iberlibro",plat:"iberlibro"},{label:"🏷️ Wallapop",plat:"wallapop"},{label:"👗 Vinted",plat:"vinted"},{label:"📦 Amazon",plat:"amazon"}].map(p => (
                    <button key={p.plat} style={{padding:"8px 12px", background:"#1e1208", border:"1px solid #2a3a5a", color:"#b89070", cursor:"pointer", fontSize:13, borderRadius:2, fontFamily:"Georgia, serif"}}
              ...
              <div style={{display:"flex", flexDirection:"column", gap:8, marginTop:16}}>
                <button style={{padding:"14px 18px", background:"#1e1208", border:"1px solid #2a3a5a", color:"#c8d8ee", cursor:"pointer", borderRadius:6, fontFamily:"Georgia, serif", fontSize:15, textAlign:"left", display:"flex", alignItems:"center", gap:10}}
          ...
          {/* EDITAR CAMPS */}
          {showEdit && (
            <div style={{ background: "#0e0a04", padding: 14, borderRadius: 4, marginBottom: 12 }}>
          ...
          {showExtra && (
            <div style={{ marginBottom: 12, padding: "12px 14px", background: "#0e0a04", borderRadius: 2 }}>
              {activeEd.traduccion && <div style={{ fontSize: 15, color: "#d0cdc0", marginBottom: 7 }}><span style={{ color: "#c8692acc", fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>Traducció </span>{activeEd.traduccion}</div>}
              {activeEd.isbn && <div style={{ fontSize: 15, color: "#d0cdc0", marginBottom: 7 }}><span style={{ color: "#c8692acc", fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>ISBN </span>{activeEd.isbn}</div>}
              {activeEd.pagines && <div style={{ fontSize: 15, color: "#d0cdc0", marginBottom: 7 }}><span style={{ color: "#c8692acc", fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>Pàgines </span>{activeEd.pagines}</div>}
              {activeEd.origen && <div style={{ fontSize: 15, color: "#d0cdc0", marginBottom: 7 }}><span style={{ color: "#c8692acc", fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>Origen </span>{activeEd.origen}</div>}
              {activeEd.preu_pagat && <div style={{ fontSize: 15, color: "#d0cdc0", marginBottom: 7 }}><span style={{ color: "#c8692acc", fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>Preu pagat </span>{activeEd.preu_pagat} €</div>}
              <div style={{ fontSize: 15, color: "#d0cdc0", marginBottom: 7 }}><span style={{ color: "#c8692acc", fontSize: 13, letterSpacing: 1, textTransform: "uppercase" }}>ID </span>{activeEd.edicion_id}</div>
            </div>
          )}
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {[
                ...
                {val:"venut",   label:"💰 Venut",    color:"#c8692a"},
                {val:"regalat", label:"🎁 Regalat",  color:"#c07840"},
                {val:"perdut",  label:"✕ Perdut",    color:"#c44"},
              ].map(e => {
                const active = (activeEd.estat_exemplar||"actiu") === e.val;
                return (
                  <button key={e.val}
                    style={{ padding:"7px 14px", background:active?e.color+"22":"transparent", border:`1px solid ${active?e.color+"66":"#2e1a08"}`, color:active?e.color:"#555", cursor:"pointer", fontSize:11, borderRadius:2, fontFamily:"Georgia, serif" }}
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 0", borderBottom:"1px solid #1a2540", marginBottom:8 }}>
            <div>
              <span style={{ fontSize:10, color:"#c8692a66", textTransform:"uppercase", letterSpacing:1 }}>Primera edició </span>
              {isPrimeraEdicio(activeEd) && <span style={{ fontSize:9, background:"#c8692a", color:"#130c06", padding:"1px 5px", fontWeight:"bold", marginLeft:6 }}>1ª ED</span>}
            </div>
            <button
              style={{ padding:"4px 12px", background: activeEd.primera_edicio===true?"#c8692a22": activeEd.primera_edicio===false?"#c4444422":"transparent", border:`1px solid ${activeEd.primera_edicio===true?"#c8692a55":activeEd.primera_edicio===false?"#c4444444":"#2e1a08"}`, color: activeEd.primera_edicio===true?"#c8692a":activeEd.primera_edicio===false?"#c44":"#555", cursor:"pointer", fontSize:10, borderRadius:2, fontFamily:"Georgia, serif" }}
              onClick={() => {
   ...
   {!deleteConfirm
      ...
      {view === "prestatge" && <>
        <div style={{...S.statsBar, display:"flex", justifyContent:"space-between", alignItems:"center"}}>
          ...
          <span style={{ fontSize:10, color:"#a07850" }}>● color = saga</span>
        </div>
        {loading ? <div style={S.loader}>CARREGANT · · ·</div>
                    ...
                    {falten.map(o => {
                      ...
                      <div key={o.obra_id} style={{...S.faltenItem, opacity: cobertPerOmnibus ? 0.65 : 1}}>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:4, alignItems:"flex-end" }}>
                          ...
                          <button style={{ background:"none", border:"1px solid #c8692a33", color:"#c8692a66", cursor:"pointer", fontSize:9, padding:"2px 6px", borderRadius:2, fontFamily:"Georgia, serif" }}
      ...
      {view === "wishlist" && (
        <div>
          <div style={{ ...S.statsBar, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <button style={{ background:"#c8692a22", border:"1px solid #c8692a44", color:"#c8692a", cursor:"pointer", fontSize:11, padding:"3px 10px", borderRadius:2, fontFamily:"Georgia, serif" }}
              onClick={() => requireAuth(() => setShowWishlistForm(!showWishlistForm))}>+ Afegir</button>
          </div>

          {/* Formulari afegir */}
          {showWishlistForm && (
            <div style={{ padding:"14px 16px", background:"#0e0a04", borderBottom:"1px solid #1a2540" }}>
              ...
              <div style={{ display:"flex", gap:6, marginBottom:8 }}>
                {[{v:1,label:"🔴 Alta"},{v:2,label:"🟡 Normal"},{v:3,label:"🟢 Baixa"}].map(p => (
                  <button key={p.v} style={{ flex:1, padding:"5px", background:wishlistForm.prioritat===p.v?"#c8692a22":"transparent", border:`1px solid ${wishlistForm.prioritat===p.v?"#c8692a44":"#2e1a08"}`, color:wishlistForm.prioritat===p.v?"#c8692a":"#555", cursor:"pointer", fontSize:10, borderRadius:2, fontFamily:"Georgia, serif" }}
                    onClick={() => setWishlistForm(f=>({...f,prioritat:p.v}))}>{p.label}</button>
                ))}
              </div>
              <div style={{ display:"flex", gap:8 }}>
                <button style={{ flex:1, padding:"8px", background:"#c8692a22", border:"1px solid #c8692a44", color:"#c8692a", cursor:"pointer", borderRadius:2, fontFamily:"Georgia, serif", fontSize:12 }}
          ...
          {wishlistItems.length === 0
            ...
            : wishlistItems.map(w => {
                const prioritatColor = w.prioritat===1?"#c44":w.prioritat===3?"#6ec88e":"#c8692a";
                  ...
                  <div key={w.wishlist_id} style={{ padding:"14px 16px", borderBottom:"1px solid #1a2540" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
                      <div style={{ flex:1 }}>
                        ...
                        {w.autor && <div style={{ fontSize:11, color:"#c8692a88", marginBottom:2 }}>{w.autor}{w.editorial && ` · ${w.editorial}`}{w.any_aprox && ` · ${w.any_aprox}`}</div>}
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:4, alignItems:"flex-end", flexShrink:0 }}>
                        ...
                        <button style={{ background:"none", border:"1px solid #8899cc33", color:"#c07840", cursor:"pointer", fontSize:9, padding:"2px 6px", borderRadius:2, fontFamily:"Georgia, serif" }}
                    ...
                    {showBuscar && (
                      <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginTop:8 }}>
                        {[
                          ...
                          <button key={p.plat} style={{ padding:"4px 8px", background:"#1e1208", border:"1px solid #1e3060", color:"#888", cursor:"pointer", fontSize:10, borderRadius:2, fontFamily:"Georgia, serif" }}
        ...
        <div style={S.statsPage}>
          ...
          {!stats ? <div style={S.loader}>CARREGANT · · ·</div> : <>
            <div style={S.statGrid}>
              ...
              {stats.venuts > 0 && <div style={{...S.statCard, border:"1px solid #c8692a33"}}><div style={{...S.statNum, color:"#c8692a"}}>{stats.venuts}</div><div style={S.statLabel}>💰 Venuts</div></div>}
              {stats.regalats > 0 && <div style={{...S.statCard, border:"1px solid #8899cc33"}}><div style={{...S.statNum, color:"#c07840"}}>{stats.regalats}</div><div style={S.statLabel}>🎁 Regalats</div></div>}
              {stats.totalVenut > 0 && <div style={{...S.statCard, border:"1px solid #c8692a33"}}><div style={{...S.statNum, color:"#c8692a"}}>{stats.totalVenut.toFixed(0)}€</div><div style={S.statLabel}>💸 Recaptat vendes</div></div>}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 20 }}>
              {[
                ...
                { label: "📌 Customs", val: edicions.filter(e => e.portada_custom).length, color: "#c8692a" },
              ].map(s => (
                <div key={s.label} style={{ background: "#1e1208", border: `1px solid ${s.color}33`, borderRadius: 4, padding: "10px 12px", textAlign: "center" }}>
                  <div style={{ fontSize: 20, color: s.color, fontWeight: "bold" }}>{s.val}</div>
            ...
            {stats.topLectures?.length > 0 && (
              <div style={{ marginTop:8 }}>
                <div style={{ fontSize:11, color:"#c8692a99", letterSpacing:1, textTransform:"uppercase", marginBottom:12 }}>⭐ Top lectures (valoració ≥ 4)</div>
                {stats.topLectures.map((e,i) => (
                  <div key={e.edicion_id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:"1px solid #111", cursor:"pointer" }} onClick={() => openObra(e)}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <span style={{ color:"#c8692a44", marginRight:8, fontSize:11 }}>#{i+1}</span>
                      <span style={{ fontSize:12, color:"#bbb" }}>{e.titulo_edicion||e.titulo_original}</span>
                    </div>
                    <div style={{ display:"flex", gap:8, alignItems:"center", flexShrink:0 }}>
                      <span style={{ fontSize:11, color:"#c8692a" }}>{"★".repeat(e.valoracio)}</span>
            ...
            {stats.topValor?.length > 0 && (
              <div style={{ marginTop: 8 }}>
                <div style={{ fontSize: 11, color: "#c8692a99", letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>💎 Top valor estimat</div>
                {stats.topValor.map((e, i) => (
                  <div key={e.edicion_id} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #111", cursor: "pointer" }} onClick={() => openObra(e)}>
                    <span style={{ fontSize: 12, color: "#bbb" }}><span style={{ color: "#c8692a44", marginRight: 8 }}>#{i+1}</span>{e.titulo_edicion}</span>
                    <span style={{ fontSize: 12, color: "#c8692a", flexShrink: 0 }}>{e.preu_mercat}€</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {stats.prestats?.length > 0 && (
              <div style={{ marginTop: 20 }}>
                <div style={{ fontSize: 11, color: "#c8692a99", letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>🔄 Llibres prestats ({stats.prestats.length})</div>
                {stats.prestats.map(e => (
                  <div key={e.edicion_id} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #111", cursor: "pointer" }} onClick={() => openObra(e)}>
                    <span style={{ fontSize: 12, color: "#bbb" }}>{e.titulo_edicion}</span>
                    <span style={{ fontSize: 11, color: "#c8692a" }}>{e.prestat_a}</span>
                  </div>
                ))}
              </div>
            )}
